...
def create_from_imagedir(tfrecord_dir, image_dir, resize, shuffle):
    #
    ...
    #
    #print('Loading images from "%s"' % image_list)
    #with open(image_list,'r') as ff:
    #    image_list_ = ff.readlines()
    #image_filenames = [fn.strip() for fn in image_list_]
    ##
    #if len(image_filenames) == 0:
    #    error('No input images found')
    #    #
        ...
        for idx in range(order.size):
            ...
            if resize:
                ...
                if nh!=hh:
                    nw=int((nh*ww)/hh)
                    import scipy.misc
                    img_res = scipy.misc.imresize(img, (nh, nw))
                    dif = int((nw-512)/2)
                    img = img_res[:,dif:-dif,:]
                    #print(img.shape)
                    #pdb.set_trace()
                    ##
                elif hh==resize and ww!=resize:
                    import scipy.misc
                    img = scipy.misc.imresize(img, (resize,resize))
            if channels == 1:
...
def create_from_imagelist(tfrecord_dir, image_list, resize, shuffle):
    #    print('Loading images from "%s"' % image_dir)
    #    image_filenames = sorted(glob.glob(os.path.join(image_dir, '*')))
    #    if len(image_filenames) == 0:
    #        error('No input images found')
...
def create_from_imagelist_withlabels(tfrecord_dir, image_list, resize, shuffle):
    #print('Loading images from "%s"' % image_dir)
    #image_filenames = sorted(glob.glob(os.path.join(image_dir, '*')))
    import pdb
    ...
    with open(image_list,'r') as ff:
        image_list_ = ff.readlines()
    image_filenames = [fn.strip().split(' ')[0] for fn in image_list_]
    image_labels = [int(fn.strip().split(' ')[1]) for fn in image_list_]
    #
    if len(image_filenames) == 0:
        error('No input images found')  
    #
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        ...
        for idx in range(order.size):
            ...
            tfr.add_image(img)
        #pdb.set_trace()
        tfr.add_labels(np.array([image_labels[idx] for idx in order]))
...
def execute_cmdline(argv):
    ...
    p.add_argument(     'image_list',        help='Directory containing the images')
    p.add_argument(     '--resize',        help='Resize image (default: 512x512)', type=int, default=512)
    p.add_argument(     '--shuffle',        help='Randomize image order (default: 1)', type=int, default=1)

    p = add_command(    'create_from_images_withlabels', 'Create dataset from a directory full of images.', 'create_from_images datasets/mydataset myimagedir')
    p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
    p.add_argument(     'image_list',        help='textfile containing the image file paths and labels')
    p.add_argument(     '--resize',        help='Resize image (default: 512x512)', type=int, default=512)
    p.add_argument(     '--shuffle',        help='Randomize image order (default: 1)', type=int, default=1)
