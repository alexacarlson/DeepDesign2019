...
def reprocess_image(x):
    ...
    #xrgb = x[:,:,::-1]
    #x = (x + 1.)*127.5
    xrgb = np.clip(x, 0, 255).astype('uint8')
    return xrgb
...
def finetune_vgg16(train_folder_path, weights_dir, image_shape, num_epochs):
    ...
    print(weights_dir)
    custom_model.save_weights(weights_dir,'vgg16_custom_model_weights.h5')
...
if vgg16_model=='imagenet':
    ...
    #
    weights_file = os.path.join(vgg16_model, 'vgg16_custom_model_weights.h5')
...
#                          #
## build the model/compute the gradient of the input picture wrt this loss
...
for input_img_data, dream_input_id in zip(img_list, dream_input_ids):
    #
    input_img_ = input_img_data
    #input_img_ = preprocess_input(input_img_data, mode ='tf')
    #input_img_arr = preprocess_input(input_img_arr, mode ='tf')
    #
    for iter_ in range(num_iters):
        print 'iteration%s'%(str(iter_))
