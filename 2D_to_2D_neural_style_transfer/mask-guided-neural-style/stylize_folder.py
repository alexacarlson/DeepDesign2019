...
def  main(args):
    ...
    for cfn, content_img, init_img in zip(content_img_filelist, content_img_list, init_img_list):
        ...
        with tf.Graph().as_default():
            ...
            if args.optimizer == 'adam':
                ...
                for i in range(args.iteration):
                    ...
                    if i % args.log_iteration == 0:
                        ...
                        #output_path = os.path.join(args.output_dir, os.path.splitext(cfn)[0] +'2'+os.path.split(os.path.splitext(args.style_img)[1])[0]+ '.png')
            ...
            result = sess.run(target_net['input'])
            output_path = os.path.join(args.output_dir, os.path.splitext(cfn)[0]+'2'+os.path.split(os.path.splitext(args.style_img)[0])[1]+ '.png')
            write_image(output_path, result)
